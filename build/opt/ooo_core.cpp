...
OOOCore::OOOCore(FilterCache* _l1i, FilterCache* _l1d, g_string& _name) : Core(_name), l1i(_l1i), l1d(_l1d), cRec(0, _name) {
    ...
    instrs = uops = bbls = approxInstrs = mispredBranches = mispredInstrs = mispredPenalty = opExecuted = 0, loadStallsTotal = 0, storeStallsTotal = 0;
...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo) {
    ...
    for (uint32_t i = 0; i < bbl->uops; i++) {
        ...
        //std::cout<<"uop->decCycle: "<<uop->decCycle<<"| prevDecCycle: "<<prevDecCycle<<"| decDiff: "<< decDiff<<"| decodeCycle: "<<decodeCycle<<"| decodeCycle + decDiff: "<<decodeCycle+decDiff<<"| uopQueue.minAlloc"<<uopQueue.minAllocCycle()<<std::endl; 
	...
	// Model RAT + ROB + RS delay between issue and dispatch
       ...
       /* if(dispatchCycle < lastCommitCycle)
            std::cout<<"*******dispatch is less than last commit*********"<<std::endl;
        else
            std::cout<<"&&&&&&dispatch is MORE than last commit&&&&&&&"<<std::endl;
       */
        ...
        // std::cout<<"Commits------- dispatch: "<<dispatchCycle<<"***other:"<<lastCommitCycleOther<<"***store: "<<lastStoreCommitCycle<<std::endl;
         // std::cout<<"store commit: "<<lastStoreCommitCycle<<std::endl;
            ...
            case UOP_STORE:
                {
                    ...
                    //top-down
                   // std::cout<<storeQueue.getIdx()<<std::endl;
                }
                break;
	...
	//top-down
/*	
        std::map<uint64_t,uint64_t>::iterator it = commitCycleMap.find(commitCycle);
        
	if (it !=commitCycleMap.end()) {
		it->second++;
                if(it->second > 3) opExecuted++;
		
	}
 	else {
		commitCycleMap.insert(std::make_pair(commitCycle, 1));
	}
    */
        // Mark retire at ROB
    ...
    if (branchPc && !branchPred.predict(branchPc, branchTaken)) {
       ...
       //top-down
        ...
        //top-down
       // std::cout<<"ReqCycle Before: "<<reqCycle<<std::endl;
        ...
        //top-down
        //std::cout<<"ReCycle After: "<<reqCycle<<std::endl;
    ...
    for (Address fetchAddr = bblAddr; fetchAddr < endAddr; fetchAddr += lineSize) {
        ...
        cRec.record(curCycle, curCycle, curCycle + fetchLat);
        //std::cout<<"fetchLat: "<<fetchLat<<std::endl;
        fetchCycle += fetchLat;
    }
    ...
    decodeCycle++;
    //std::cout<<"decodeCycle++"<<std::endl;
    uint64_t minFetchDecCycle = fetchCycle + (DECODE_STAGE - FETCH_STAGE);
    if (minFetchDecCycle > decodeCycle) {
#ifdef OOO_STALL_STATS
       //nika: I am fixing this, because the result is negative number.It is saying if fetch rules, 
       //take the diff into account, so we should reverse the sub.
       //profFetchStalls.inc(decodeCycle - minFetchDecCycle);
	
        profFetchStalls.inc(minFetchDecCycle - decodeCycle);

	//std::cout<<"decode cycle: "<<decodeCycle<<" *** minFetchDecCycle: "<<minFetchDecCycle<<std::endl;
#endif
        decodeCycle = minFetchDecCycle;
        //std::cout<<"minFetchDecCycle: "<<minFetchDecCycle<<std::endl;
    }
...
//top-down
/*    std::cout<<"I am outputting mapppppppppppppppppppppppppppp"<<std::endl;
   
    for(auto &pair: commitCycleMap){ 
    	std::cout<<"map1: "<<pair.first<<" *** map2: "<<pair.second<<std::endl;
	if(pair.second <3) opExecuted++;
    }
*/
...
void OOOCore::advance(uint64_t targetCycle) {
    ...
    //std::cout<<"decodeCycle in advance: "<<decodeCycle<<std::endl;
